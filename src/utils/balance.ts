...
export async function waitForMinimumUsdcBalance(
    ...
    while (true) {
        try {
            ...
            if (logEveryPoll) {
                console.log(`[INFO] USDC gate: available=${available.toFixed(6)} (balance=${balance.toFixed(6)}, allowance=${allowance.toFixed(6)}), required>=${minimumUsd}`);
            }

            if (ok) {
                console.log(`[SUCCESS] USDC gate passed: available=${available.toFixed(6)} >= ${minimumUsd}`);
                return { ok: true, available, balance, allowance };
            }
        } catch (error) {
            console.log(`[WARNING] USDC gate check failed: ${error instanceof Error ? error.message : String(error)}`);
        }

        if (timeoutMs > 0 && Date.now() - start >= timeoutMs) {
            console.log(`[ERROR] USDC gate timed out after ${Math.round(timeoutMs / 1000)}s (required>=${minimumUsd})`);
